...
import io.m10s.kss.model.PodResourceDetails
...
class K8sServiceReader(
    ...
    ): List<io.m10s.kss.model.ServiceSummary> {
    ...
    private fun fetchServiceSummariesForNamespace(
        ...
        return services
            ...
            .map { service ->
                ...
                io.m10s.kss.model.ServiceSummary(
    ...
    fun fetchDeploymentHistory(
        ...
        namespace: String = resolveCurrentNamespace()
    ): io.m10s.kss.model.DeploymentHistoryResponse {
        ...
        val revisions = replicaSets
            .mapNotNull { rs ->
                ...
                io.m10s.kss.model.DeploymentRevision(
                    revision = rev,
                    image = image,
        ...
        return io.m10s.kss.model.DeploymentHistoryResponse(
    ...
    fun rollbackDeployment(
        ...
        namespace: String,
        targetRevision: Int?
    ): io.m10s.kss.model.RollbackResponse {
        try {
            ...
            if (targetRev == history.currentRevision) {
                return io.m10s.kss.model.RollbackResponse(false, "Cannot rollback to current revision")
            }

            val targetRevisionData = history.revisions.find { it.revision == targetRev }
                ?: return io.m10s.kss.model.RollbackResponse(false, "Target revision $targetRev not found in history")
            ...
            } ?: return io.m10s.kss.model.RollbackResponse(false, "ReplicaSet for revision $targetRev not found")
            ...
            return io.m10s.kss.model.RollbackResponse(
                success = true,
                message = "Successfully rolled back to revision $targetRev",
                targetRevision = targetRev
            )
        } catch (e: Exception) {
            return io.m10s.kss.model.RollbackResponse(false, "Rollback failed: ${e.message}")
        }
    }
    ...
    fun restartDeployment(
        serviceName: String,
        namespace: String
    ): io.m10s.kss.model.RestartResponse {
        try {
            ...
            return io.m10s.kss.model.RestartResponse(
                success = true,
    ...
    fun fetchServiceEndpoints(
        serviceName: String,
        namespace: String = resolveCurrentNamespace()
    ): io.m10s.kss.model.ServiceEndpointsResponse {
        // 1. Get service info
        ...
        val endpoints = try {
            ...
            val endpointsList = mutableListOf<io.m10s.kss.model.ServiceEndpoint>()
            ...
            endpointsObj.subsets?.forEach { subset ->
                ...
                subset.addresses?.forEach { addr ->
                    ...
                    ports.forEach { port ->
                        endpointsList.add(
                            io.m10s.kss.model.ServiceEndpoint(
                                ip = addr.ip ?: "unknown",
                                port = port,
                                podName = podName,
                                ready = true
        ...
        val ingresses = try {
            ...
            val ingressRules = mutableListOf<io.m10s.kss.model.IngressRule>()
        ...
        return io.m10s.kss.model.ServiceEndpointsResponse(
            serviceName = serviceName,
            namespace = namespace,
    ...
    fun fetchPodEvents(
        podName: String,
        namespace: String = resolveCurrentNamespace()
    ): List<io.m10s.kss.model.PodEvent> {
        return try {
    ...
    fun fetchServiceEvents(
        ...
        val allEvents = pods.flatMap { pod ->
            val podName = pod.metadata?.name ?: return@flatMap emptyList<io.m10s.kss.model.PodEvent>()
    ...
    ): io.m10s.kss.model.PodLogsResponse {
        try {
            ...
            return io.m10s.kss.model.PodLogsResponse(
                podName = podName,
    ...
    fun fetchServiceConfigs(
        serviceName: String,
        namespace: String
    ): io.m10s.kss.model.ServiceConfigsResponse {
        try {
            ...
            val configMapRefs = mutableMapOf<String, io.m10s.kss.model.ConfigReference>()
            val secretRefs = mutableMapOf<String, io.m10s.kss.model.ConfigReference>()
            ...
            pods.forEach { pod ->
                ...
                volumes.forEach { volume ->
                    ...
                    volume.configMap?.let { configMap ->
                        ...
                        configMapRefs[name] = io.m10s.kss.model.ConfigReference(
                            name = name,
                            type = "ConfigMap",
                            mountPath = mountPath,
                            keys = keys,
                    ...
                    volume.secret?.let { secret ->
                        ...
                        secretRefs[name] = io.m10s.kss.model.ConfigReference(
                            name = name,
                            type = "Secret",
                            mountPath = mountPath,
                            keys = keys,
            ...
            return io.m10s.kss.model.ServiceConfigsResponse(
                serviceName = serviceName,
                namespace = namespace,
    ...
    fun fetchConfigMapData(
        name: String,
        namespace: String
    ): io.m10s.kss.model.ConfigMapData {
        return try {
    ...
    fun fetchSecretValue(
        ...
        namespace: String,
        key: String
    ): io.m10s.kss.model.SecretValueResponse {
        val secret = coreApi.readNamespacedSecret(name, namespace).execute()
        ...
        return io.m10s.kss.model.SecretValueResponse(key = key, encodedValue = encoded)
    }
    ...
    ): io.m10s.kss.model.ScaleResponse {
        if (minReplicas < 1) {
            return io.m10s.kss.model.ScaleResponse(
                success = false,
